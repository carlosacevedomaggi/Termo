...
export async function POST(request: Request) {
  ...
  const { name, email, address, method, currency, amount, items } = data as {
    name: string; email: string; address: string; method: string; currency: string; amount: number; items: { id: string; name: string; price: number; quantity: number }[]
  }
