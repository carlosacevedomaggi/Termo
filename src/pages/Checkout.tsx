...
import { PAYMENT_METHODS, DEFAULT_EXCHANGE_RATE_VES_PER_USD } from '@/shared/payments'
...
export default function Checkout() {
  ...
  const vesApprox = Math.round(total * DEFAULT_EXCHANGE_RATE_VES_PER_USD)
  ...
  return (
    <div className="container-edge py-14 grid lg:grid-cols-2 gap-10">
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          <div className="grid gap-4">
            <div>
              ...
              <input
                className="mt-1 w-full rounded-lg border border-border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-black"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFieldValues(v => ({ ...v, address: e.target.value }))}
              />
          ...
          <div className="space-y-3">
            ...
            <div className="border border-border rounded-xl p-4">
              ...
              <div className="text-xs opacity-70 mt-3">
                After sending the payment, fill in the details below and place your order.
              </div>
            </div>
            ...
            <div className="grid gap-4">
              {selectedMethod.fields.map(f => (
                <div key={f.id}>
                  ...
                  {f.type === 'select' ? (
                    <select
                      className="mt-1 w-full rounded-lg border border-border px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-black"
      ...
      <div className="border border-border rounded-xl p-5 h-fit">
        ...
        <div className="mt-2 text-xs opacity-70">Approx VES: Bs. {vesApprox.toLocaleString('es-VE')}</div>
        {selectedMethod.currency === 'USDT' && (
          <div className="mt-2 text-xs opacity-70">Approx USDT: {(total).toFixed(2)} USDT</div>
        )}
      </div>
    </div>
  )
